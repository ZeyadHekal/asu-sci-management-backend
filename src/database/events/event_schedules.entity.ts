import { Entity, Column, ManyToOne, JoinColumn, ManyToMany, JoinTable, PrimaryColumn, OneToMany, CreateDateColumn, UpdateDateColumn } from 'typeorm';
...
export class EventSchedule extends ManagementEntity {
	...
	@Expose()
	examModels?: string; // JSON array of exam model file URLs
...
export class StudentEventSchedule extends OmitType(ManagementEntity, ['id']) {
	...
	@Column({ type: 'text', nullable: true })
	@Expose()
	submittedFiles?: string; // JSON array of submitted file URLs

	@Column({ nullable: true })
	@Expose()
	assignedExamModelUrl?: string; // URL of the assigned exam model
