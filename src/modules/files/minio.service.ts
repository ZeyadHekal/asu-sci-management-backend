...
export class MinioService implements OnModuleInit {
	...
	async uploadFile(file: Express.Multer.File, prefix = ''): Promise<string> {
		try {
			const objectName = prefix ? `${prefix}/${Date.now()}-${file.originalname}` : `${Date.now()}-${file.originalname}`;
	...
	async getPresignedUrl(objectName: string, expiry?: number): Promise<string> {
		try {
			const expiryTime = expiry || parseInt(this.configService.get<string>('MINIO_SIGNED_URL_EXPIRY', '3600'));

			return await this.minioClient.presignedGetObject(this.bucketName, objectName, expiryTime);
		} catch (error) {
