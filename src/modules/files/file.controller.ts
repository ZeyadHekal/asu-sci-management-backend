import { Controller, Delete, Get, Param, ParseIntPipe, Post, Req, Res, UploadedFile } from '@nestjs/common';
...
export class FileController {
	...
	async uploadFile(@UploadedFile() file: Express.Multer.File, @Req() req: Request & { fileOptions?: FileUploadOptions }): Promise<FileResponseDto> {
		return this.fileService.uploadFile(file, req.fileOptions || {});
	}
	...
	async getFileById(@Param('id', ParseIntPipe) id: number, @SignedUrl() { expirySeconds }: { expirySeconds?: number }): Promise<FileResponseDto> {
	...
	@ApiParam({ name: 'id', description: 'File ID' })
	async getSignedUrl(@Param('id', ParseIntPipe) id: number, @SignedUrl() { expirySeconds }: { expirySeconds?: number }): Promise<{ url: string }> {
