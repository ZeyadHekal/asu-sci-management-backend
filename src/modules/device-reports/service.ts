...
export class DeviceReportService {
    ...
    async create(createDto: imports.CreateDto, userId?: UUID): Promise<imports.GetDto> {
        ...
        const report = this.repository.create({ ...createDto, reporterId, status: ReportStatus.PENDING_REVIEW });
    ...
    async rejectReport(reportId: UUID, reason: string): Promise<imports.GetDto> {
        const report = await this.repository.findOneBy({ id: reportId });
        if (!report) {
            throw new NotFoundException('Report not found');
        }
        ...
        report.fixMessage = reason; // Store the rejection reason in the fixMessage field
        await this.repository.save(report);

        // Emit WebSocket event for report status change
        const deviceReportStatusChangedData: DeviceReportStatusChangedData = {
            reportId: report.id,
